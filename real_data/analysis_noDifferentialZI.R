...
plot_distributions <- function(data, focus){
  p <- data.frame(t(data$Y[focus,]), "Group" = data$group_names[data$X[2,]+1]) %>% 
    ...
    #stat_binline(rel_min_height=0.001, alpha=0.7, draw_baseline = FALSE, scale=0.97, bins=7) +
    scale_x_log10() +
    #theme_ridges() +
    theme_minimal()+
...
plot_rank <- function(fitZI, fitNZI, fn){
  ...
  write.table(res, file=paste0(fn,"_intersect.tsv"))
  p <- as.data.frame(res) %>% 
...
## This one makes combined figures
...
# This one makes individual figures
run_and_save <- function(data, fn){
  fitZI <- fitZI_zinbwave(data)
  fitNZI <- fitNZI_zinbwave(data)
  summary_fits <- summarize_fits(fitZI, fitNZI)
  ...
  p <- plot_DE(summary_fits)
  ffn <- paste0("DE_", fn, ".pdf")
  ggsave(ffn, plot=p, height=4, width=3.5, units="in")
  
  p <- plot_distributions(data, focus)
  ffn <- paste0("hist_", fn, ".pdf")
  ggsave(ffn, plot=p, height=4, width=3.5, units="in")
